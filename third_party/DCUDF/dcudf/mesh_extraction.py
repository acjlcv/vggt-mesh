...
import os
from third_party.DCUDF.dcudf.VectorAdam import VectorAdam
import warnings
...
class dcudf:
    ...
    Args:
        query_function:       differentiable function, input tensor[batch_size, 3] and output tensor[batch_size]
        resolution:
        threshold:
        max_iter:
    ...
    def __init__(
        ...
        if bound_min is None:
            bound_min = torch.tensor(
                [-1 + self.threshold, -1 + self.threshold, -1 + self.threshold],
                dtype=torch.float32,
            )
    ...
    def optimize(self):
        ...
        # u = np.load('grid_udf_test.npy')

        self.mesh = threshold_MC(
            u,
            self.threshold,
            self.resolution,
        ...
        # self.mesh = trimesh.load('mesh_after_mc.obj')
        ...
        for it in range(self.max_iter):
            ...
            epoch_loss = torch.tensor(0, dtype=torch.float64).cuda()
            ...
            while head < num_samples:
                ...
                df = query_function(sample_subset, sample_subset)
                df_loss = df.mean()
                loss = df_loss
                ...
                if it <= self.normal_step:
                    ...
                    face_weight = s_value[
                        vertex_faces[head : min(head + self.max_batch, num_samples)]
                    ]

                    face_weight[
                        ~face_mask[head : min(head + self.max_batch, num_samples)]
                    ] = 0
            ...
            while mid_head < mid_num_samples:
                ...
                sub_mid_points = mid_points[
                    mid_head : min(mid_head + self.max_batch, mid_points.shape[0])
                ]
                mid_df = query_function(sub_mid_points, sub_mid_points)
                mid_df_loss = mid_df.mean()
                loss = mid_df_loss.cuda()
                if it > self.normal_step:
                    offset = (
                        mid_points[
                            mid_head : min(
                                mid_head + self.max_batch, mid_points.shape[0]
                            )
                        ]
                        - origin_points[
                            mid_head : min(
        ...
        final_mesh = trimesh.Trimesh(
            vertices=xyz.detach().cpu().numpy(), faces=self.mesh.faces, process=False
        )
        ...
        if self.is_cut == 1:
            ...
            if final_mesh_cuple is not None:
                ...
                print(
                    "It seems that model is too complex, cutting failed. Or just rerunning to try again."
                )
    ...
    def extract_fields(self):
        ...
        N = 32
        X = torch.linspace(-0.5, 0.5, self.resolution).split(N)
        Y = torch.linspace(-0.5, 0.5, self.resolution).split(N)
        Z = torch.linspace(-0.5, 0.5, self.resolution).split(N)
...
class Dcudf_on_UDF:
    """
    mesh extraction on Geometric UDF learning model
    ...
    def optimize(self):
        ...
        # u_learnt = self.extract_fields_fast()
        # u = np.load(self.udf_field_path)
        # u_grid = np.reshape(self.udf_field, (256, 256, 256))
        u_grid = self.udf_field.reshape(256, 256, 256)
        # turn to cpu
        ...
        # init points
        xyz = torch.from_numpy(self.mesh.vertices.astype(np.float32))
        ...
        for it in tqdm(range(self.max_iter)):
            ...
            # xyz = xyz[~torch.any(xyz.isnan(),dim=1)]
            ...
            if it == self.normal_step:
                ...
                normals = torch.FloatTensor(normal_mesh.face_normals)
                normals = normals.to(self.device)
                origin_points = get_mid(xyz, self.mesh.faces).detach().clone()

            self.update_learning_rate(it)

            epoch_loss = torch.tensor(0, dtype=torch.float64)
            ...
            while head < num_samples:
                ...
                # udf, valid_index, _ = self.query_function(
                #     self.input_pcd.unsqueeze(0), sample_subset.T.unsqueeze(0))
                # df = udf[valid_index[:,0], valid_index[:,1]]

                df = self.query_function(
                    sample_subset.unsqueeze(0),
                    sample_subset.T.unsqueeze(0),
                    self.device,
                )

                # df = self.query_function(sample_subset)
                ...
                if it <= self.normal_step:
                    ...
                    lap_v = laplacian_step(laplacian_op, xyz.to(self.device))
                    lap_v = torch.mul(lap_v, lap_v)
                    lap_v = lap_v[head : min(head + self.max_batch, num_samples)]
                    laplacian_loss = face_weight * torch.sum(lap_v, dim=1)
                    laplacian_loss = self.laplacian_weight * laplacian_loss.mean()
            ...
            while mid_head < mid_num_samples:
                ...
                # mid_udf, valid_index_mid, _ = self.query_function(
                #     self.input_pcd.unsqueeze(0), sub_mid_points.T.unsqueeze(0))

                # mid_df = mid_udf[valid_index_mid[:,0], valid_index_mid[:,1]]

                mid_df = self.query_function(
                    sub_mid_points.unsqueeze(0),
                    sub_mid_points.T.unsqueeze(0),
                    self.device,
                )

                # mid_df = self.query_function(sub_mid_points)

                mid_df_loss = mid_df.mean()

                loss = mid_df_loss.to(self.device)
                if it > self.normal_step:
                    offset = (
                        mid_points[
                            mid_head : min(
        ...
        if self.is_cut == 1:
            from third_party.DCUDF.dcudf.mesh_cut import mesh_cut
    ...
    def extract_fields(self):
        ...
        N = 16
        X = torch.linspace(self.bound_min[0], self.bound_max[0], self.resolution).split(
            N
        )
        Y = torch.linspace(self.bound_min[1], self.bound_max[1], self.resolution).split(
            N
        )
        Z = torch.linspace(self.bound_min[2], self.bound_max[2], self.resolution).split(
            N
        )
        ...
        for xi, xs in enumerate(X):
            for yi, ys in enumerate(Y):
                for zi, zs in enumerate(Z):
                    ...
                    # val, _, valid_K_count = self.query_function(self.input_pcd.unsqueeze(0), pts.T.unsqueeze(0))
    ...
    def extract_fields_fast(self):

        x = torch.linspace(self.bound_min[0], self.bound_max[0], self.resolution)
        y = torch.linspace(self.bound_min[1], self.bound_max[1], self.resolution)
        z = torch.linspace(self.bound_min[2], self.bound_max[2], self.resolution)

        # Create meshgrids for x, y, z dimensions
        X, Y, Z = torch.meshgrid(x, y, z, indexing="ij")

        # Stack and reshape to get a list of xyz coordinates
        xyz = torch.stack((X, Y, Z), dim=-1).reshape(-1, 3)

        grid_split = torch.split(xyz, 10000)
        udf_list = []

        for i, query in enumerate(grid_split):
            udf = self.query_function(
                self.input_pcd.unsqueeze(0),
                query.T.unsqueeze(0).to(self.device),
                self.device(),
            )
            udf_list.append(udf.detach().cpu())

        concatenated_udf = torch.cat(udf_list, dim=-1)

        return (
            concatenated_udf.squeeze(0)
            .reshape((self.resolution, self.resolution, self.resolution))
            .detach()
            .cpu()
            .numpy()
        )
